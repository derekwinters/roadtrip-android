...
| AND-007 | When `GET /api/profiles` returns an empty list, the profile picker shows a first-run "Set up your family" flow that creates the first profile (parent role enforced by the flow) and signs in as them; with one or more profiles the picker is the select-only avatar grid with no create affordance. The flow collects only a name — there is no avatar input; the create request carries no avatar and the server assigns its default. | auto |
...
| AND-009 | A failed first-run create never dead-ends the wizard on a raw server error. A 401 `unauthenticated` refusal means the server's bootstrap is closed, so the flow re-probes `GET /api/profiles` immediately: profiles present leaves setup for the select-only grid (setup finished elsewhere — nothing is auto-selected); a still-empty list shows an actionable message that the trip server doesn't support first-run setup yet (older server version — update it, then retry). Transport failures (create or re-probe) show a human offline message. Every failure leaves the wizard intact and retryable. | auto |
