...
| AND-013 | Single-choice selectors present their options as wrapping single-select choice chips: a `FlowRow` of Material 3 `FilterChip`s so options wrap and never overlap, clip, or truncate mid-word — regardless of option count, label length, narrow width, or font scale. The selected option is unmistakable — a filled/tonal chip plus a leading `Done` check — while unselected options are outlined and clearly read as inactive. Applied consistently across the game type and mode pickers (new-game dialog, ANDGAME-002) and the Kid/Parent role pickers (profile picker, profile admin) via one shared `SingleChoiceChips` composable. The ordered option sets with their labels are shared core models — `RoleChoices` (Kid, Parent), `GameTypeChoices`, and `GameModeChoices` — so every selector renders the same set and labels and the game-type display label has a single source of truth. The invited-profile `RadioButton` picker is unaffected. | auto |
| AND-014 | First launch has no baked-in server address: when no `server_url` is stored the app shows an explicit "Enter server address" setup screen before the profile picker (AND-001) and before any API call — no request is made to an assumed host (e.g. `10.0.0.2:8080`) until the user provides an address. The field starts empty; an emulator-host hint may appear only as placeholder/help text, never as a pre-filled working default. The app does not proceed until a valid address is entered. Once a valid address is stored the gate never reappears on later launches, the app stays fully usable offline (AND-005), and the address remains editable in Settings (ANDSET) and the pre-sign-in editor (AND-008). Whether setup is needed is a pure predicate over the stored value (`ServerAddress.needsSetup(stored)` — true when null/blank). | auto |
| AND-015 | The typed server address is validated before it is saved: a pure seam (`ServerAddress.validate(input)`) trims the input and accepts only a well-formed absolute `http(s)` URL — exactly what the HTTP client will accept — returning the normalized (trimmed) address to persist, or a human-readable rejection reason. Blank input and malformed input (no scheme, garbage) are rejected with a clear message and nothing is stored; a valid address is normalized, saved, and used for subsequent calls. The setup screen (AND-014), the pre-sign-in editor (AND-008), and Settings (ANDSET) all persist through this seam so an invalid address can never be stored. | auto |
