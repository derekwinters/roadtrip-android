...
- **Resolved commands** (rendered per-repo from the distribution registry):
  - Test/red-green loop command: `./gradlew -p core test`
  - Validate/verify/build command: `./scripts/validate-specs.sh`
...
- Run with `./gradlew -p core test` (or the repo's resolved test command).
...
### 5. Validate
- Run the repo's validator/build: `./scripts/validate-specs.sh` (or the
...
- `./scripts/validate-specs.sh` (or resolved validator) and the full suite pass ✓
